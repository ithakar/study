...
import algori.WeightedQuickUnionUF;

public class Percolation {
    
    WeightQuickUnion grid;
	private WeightQuickUnion auxGrid;
    ...
    public Percolation(int N) {
        ...
        grid    = new WeightQuickUnion(siteCount + 2);
        auxGrid = new WeightQuickUnion(siteCount + 1);
    ...
    public boolean isFull(int i, int j) {
        ...
        int idx = xyToIndex(i, j);
        return grid.connect(0, idx) && auxGrid.connect(0, idx);
    }

    // does the system percolate?
    public boolean percolates() {
        // Check whether virtual top and bottom sites are connected
        return grid.connect(0, state.length-1);
    }
}